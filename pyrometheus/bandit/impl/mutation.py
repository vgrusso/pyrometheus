...
class MutationMechanism(BaseMechanism):
    ...
    def species_names(self):
        ...
        return [self.namespace.mix.speciesName(i) for i in range(self.num_species)]
    ...
    def participation_set(self, species_id: Union[int, str]) -> Tuple[List[int]]:
        ...
        fwd_set = [
            i for i, r in enumerate(self.reactions()) if species_index in r.reactants
        ]
        rev_set = [
            i for i, r in enumerate(self.reactions()) if species_index in r.products
        ]
    ...
    def make_rate_coefficient(self, reaction_index, hardcode_params):
        ...
        if hardcode_params:
            ...
            k_fwd = make_arrhenius(reaction_index=reaction_index, params=params)
        else:
            params = np.array(
                [
            ...
            k_fwd = make_arrhenius(reaction_index=reaction_index)

        if temp == "translational":
            return k_fwd, params
        else:
            k_fwd.expr = substitute(k_fwd.expr, {"temperature": _temp_map[temp]})
            return k_fwd, params
    ...
    def make_species_production_rate(self, species_index):
        ...
        stoich_fwd = [self.reactants(r).count(species_index) for r in fwd_set]
        stoich_rev = [self.products(r).count(species_index) for r in rev_set]
        return species_production_rate_expr(
            species_index, fwd_set, rev_set, stoich_fwd, stoich_rev
        )
